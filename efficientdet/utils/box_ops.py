...
def encode_boxes(gt_boxes, anchors):
    ...
    """
    # Normalize center offsets by anchor dimensions so the target is scale-invariant
    t_x = (gt_boxes[:, 0] - anchors[:, 0]) / anchors[:, 2]
    t_y = (gt_boxes[:, 1] - anchors[:, 1]) / anchors[:, 3]
    # Log-space encoding keeps size targets symmetric and guarantees decoded sizes stay positive
...
def decode_boxes(deltas, anchors):
    ...
    """
    # Insert batch dim so (N, 4) anchors broadcast against (B, N, 4) deltas
    a = anchors[None]
    # Inverse of encode: delta * anchor_size + anchor_center
    pred_cx = deltas[..., 0] * a[..., 2] + a[..., 0]
    pred_cy = deltas[..., 1] * a[..., 3] + a[..., 1]
    # exp inverts the log encoding and ensures decoded sizes are always positive
